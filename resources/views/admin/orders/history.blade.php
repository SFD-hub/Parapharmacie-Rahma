<x-admin-layout title="Commandes">
    <x-admin.orders.tabs active="history" />

    <div class="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
        <x-admin.stat-card icon="check" label="Livrées" :value="$stats['delivered']" color="emerald" />
        <x-admin.stat-card icon="chart-bar" label="Chiffre d'affaires" :value="number_format($stats['delivered_amount'], 0, ',', ' ').' FCFA'" color="blue" />
        <x-admin.stat-card icon="exclamation" label="Annulées" :value="$stats['cancelled']" color="gray" />
        <x-admin.stat-card icon="archive" label="Remboursées" :value="$stats['refunded']" color="red" />
    </div>
    ...
    <form method="GET" class="mb-4 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
        ...
        @if (request()->anyFilled(['search', 'status', 'payment_method']))
            <a href="{{ route('admin.orders.history') }}" class="text-sm text-gray-500 hover:text-gray-700">Réinitialiser</a>
        @endif
    </form>
                        ...
                        <tr>
                            <td colspan="9">
                                <x-empty-state
                                    ...
                                    description="Les commandes livrées, annulées ou remboursées apparaîtront ici."
                                    icon="archive"
                                />
                            </td>
                        </tr>
