...
<div {{ $attributes->class(['group flex flex-col overflow-hidden rounded-2xl border border-gray-100 bg-white transition duration-300 hover:-translate-y-1 hover:shadow-lg']) }}>
    <a href="{{ route('products.show', $product) }}" class="relative block aspect-square overflow-hidden bg-gray-50">
        <div class="absolute left-2 top-2 z-10">
            @if ($rank)
                <span class="flex h-7 w-7 items-center justify-center rounded-full bg-primary-600 text-xs font-bold text-white shadow-sm">
                    {{ $rank }}
                </span>
            @elseif ($isExpired)
        ...
        @if ($image)
            <img
                src="{{ $image->path }}"
                alt="{{ $product->name }}"
                class="h-full w-full object-contain p-6 transition duration-300 group-hover:scale-105 {{ $unavailable ? 'opacity-50' : '' }}"
                loading="lazy"
            >
        @else
    ...
    <div class="flex flex-1 flex-col items-center gap-1 p-4 text-center">
        ...
        <a href="{{ route('products.show', $product) }}" class="line-clamp-2 text-sm font-semibold text-gray-900 group-hover:text-primary-600">
            {{ $product->name }}
        </a>
